...
import shinsai_game_ai as shinsai_game
...
def game_setup():
	...
	player_1 = shinsai_game.Player('Marina')
	player_2 = shinsai_game.Player('Lucashino')
	player_3 = shinsai_game.Player('Ariel')
	player_4 = shinsai_game.Player('Luna')
...
def resolve_turn(whose_turn, card):
	...
	# prinf'{whose_turn} played the {card}.')
	...
	for player in players:
		if player.round_points == 26:
			...
			with open(TRACK_FILE) as f:
				update = json.load(f)
			update['player_stats'][f'{player}']['shot_moon'] += 1
			update['game_stats']['shot_moon'] += 1
...
def update_stats():
	...
	for player in players:
		with open(TRACK_FILE) as f:
			update = json.load(f)
		update['player_stats'][f'{player}']['games_played'] += 1
		update['player_stats'][f'{player}']['hands_played'] \
			+= the_game.hand_num
		update['player_stats'][f'{player}']['total_score'] += player.points
		with open(TRACK_FILE, 'w') as f:
			json.dump(update, f, indent=4)
...
def determine_winner():
	...
	if len(winners) == 1:
		...
		print()
		for player in players:
			print(f'{player} got {player.points} points.')
		print(
		...
		for winner in winners:
			with open(TRACK_FILE) as f:
				update = json.load(f)
			update['player_stats'][f'{winner}']['total_ties'] += 1
			with open(TRACK_FILE, 'w') as f:
				json.dump(update, f, indent=4)
		with open(TRACK_FILE) as f:
			update = json.load(f)
		update['game_stats']['total_ties'] += 1
		with open(TRACK_FILE, 'w') as f:
				json.dump(update, f, indent=4)
		# input('Tie occurred!')
	if len(winners) >= 3:
	...
	with open(TRACK_FILE) as f:
		update = json.load(f)
	update['game_stats']['total_games'] += 1
	update['game_stats']['total_hands'] += the_game.hand_num
	update['game_stats']['winner_score'] += winner.points
	for winner in winners:
		update['player_stats'][f'{winner}']['games_won'] += 1
	with open(TRACK_FILE, 'w') as f:
		json.dump(update, f, indent=4)
...
def set_up_tracking():
	...
	for player in players:
		try:
			player_tracking = update['player_stats'][f'{player}']
		except:
			update['player_stats'][f'{player}'] = {
				'games_played': 0, 'games_won': 0, 'shot_moon': 0, 
				'hands_played': 0, 'total_score': 0, 'total_ties': 0
				}
		try:
			game_stats = update['game_stats']
		except:
			update['game_stats'] = {
				'total_games': 0, 'total_hands': 0, 'total_points': 0,
				'shot_moon': 0, 'winner_score': 0, 'total_ties': 0
				}
	with open(TRACK_FILE, 'w') as f:
		json.dump(update, f, indent=4)		

	
the_deck, the_game = shinsai_game.Deck(), shinsai_game.Game()
player_1, player_2, player_3, player_4, players, point_tracking = game_setup()
the_game.create_tracking(players, TRACK_FILE)
set_up_tracking()
start_play()